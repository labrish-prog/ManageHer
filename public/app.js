    ...
    <div class="grid split">
      <section class="card panel section">
        <div class="section-head">
          <div>
            <h3>Sales against target</h3>
            <p>Weighted forecast compared with the current sales target.</p>
          </div>
          <span class="badge gold">${Math.min(999, Number(d.targetProgress || 0))}% of target</span>
        </div>
        ${salesTargetChart(d)}
      </section>
      <section class="card panel section">
        <div class="section-head">
          <div>
            <h3>Activity stoplight</h3>
            <p>Green is healthy, yellow needs attention, red is at risk.</p>
          </div>
        </div>
        ${activityStoplight(d.activityStoplight || [])}
      </section>
    </div>
    <div class="grid split">
      <section class="card panel section">
...
function salesTargetChart(d) {
  const target = Math.max(Number(d.salesTarget || 0), 1);
  const won = Number(d.wonValue || 0);
  const forecast = Number(d.weightedForecast || 0);
  const pipeline = Number(d.pipelineValue || 0);
  const rows = [
    ["Won", won, "green"],
    ["Weighted forecast", forecast, "gold"],
    ["Open pipeline", pipeline, "pink"]
  ];
  return `
    <div class="target-summary">
      ...
      <div>
        <span>Gap</span>
        <strong>${money.format(Math.max(target - forecast, 0))}</strong>
      </div>
    </div>
      ...
      ${rows.map(([label, value, tone]) => {
        ...
        return `
          <div class="target-row">
            ...
            <div class="target-track" aria-label="${label} ${width}% of target">
              <span class="target-fill ${tone}" style="width: ${width}%"></span>
...
function activityStoplight(items) {
  const fallback = [
    { color: "green", label: "On track", value: 0, detail: "No active activity data yet" },
    { color: "yellow", label: "Needs attention", value: 0, detail: "No due items yet" },
    { color: "red", label: "At risk", value: 0, detail: "No overdue items yet" }
  ];
  return `
    <div class="stoplight-list">
      ${(items.length ? items : fallback).map((item) => `
        <div class="stoplight-item ${esc(item.color)}">
          <span class="stoplight-dot" aria-hidden="true"></span>
          <div>
            <strong>${esc(item.label)}: ${Number(item.value || 0)}</strong>
            <p>${esc(item.detail || "")}</p>
          </div>
        </div>
      `).join("")}
